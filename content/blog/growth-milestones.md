...
To give an example of how applicable APIs are, I did frontend for an NHS portal for my school throughout my senior year. A lot of what I did, whether it be logging the user in, updating the user state (in Pinia), displaying the user's information, allowing them to send forms, search for announcements, or change their profile pictures, depended on an API I pulled from in order to make sure the data made its way to the user. The API was more complicated than the one I used for my SAT project; I did it in Nuxt, a full-stack framework, instead of Vite, and I used specific fetch calls to get data from the API. On top of that, I needed a bearer token for the calls to work, and the function for calling the API needed specific parameters depending on what data I was getting and what I wanted to do with the data (GET, POST, etc.) The software I used to view and test the API was called Bruno, which is like an open-source Postman; this made it much easier for me to work with the API. This API project, in retrospect, helped prepare me to tackle a major aspect of web development, and a lot of the concepts I grasped working on it showed up while I worked on the NHS portal. 
